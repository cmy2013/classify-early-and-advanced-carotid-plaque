...
batchType=c(rep(,),rep(,),rep(,)) 
modType=c(rep("",),rep("",),rep("",),rep("",),rep("",),rep("",)) 
...
carotictr <- read.table("",header=TRUE,row.names="Tag",sep=",")
##caroticte <- read.table("",header=TRUE,row.names="Tag",sep=",")
carotrain <- data.frame(carotictr)
##carotest <- data.frame(caroticte)
x <- as.matrix(carotrain[, 2:100])
y <- carotrain[, 1]

###lasso

##newx <- as.matrix(carotest[, 2:100])
...
#####
traintrian <- read.table("",header=TRUE,row.names="Tag",sep=",")
...
###
#qda.fit<- qda (Disease_Group~.,data = train_set)
#plot()
